            ...
            >
                {projects.map((project, index) => (
                    <motion.div
                        ...
                        className="group relative rounded-2xl overflow-hidden bg-white dark:bg-[#0f172a]/60 backdrop-blur-md transition-all duration-400 hover:scale-[1.02] border border-slate-200 dark:border-slate-700/50 hover:border-indigo-400 dark:hover:border-indigo-500/30 shadow-md dark:shadow-lg hover:shadow-xl dark:hover:shadow-xl"
                    >
                        {/* Hover Overlay */}
                        <div className="absolute inset-0 bg-white/60 dark:bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col justify-center items-center gap-4 backdrop-blur-sm">
                            <div className="flex gap-4">
                                {project.links.github && (
                                    <a
                                        ...
                                        className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white transition-all border border-slate-300 dark:border-slate-600 shadow-md hover:scale-105"
                                    >
                        ...
                        {/* Image Container */}
                        <div className="relative h-64 overflow-hidden bg-slate-100 dark:bg-slate-800 rounded-t-2xl">
                        ...
                        <div className="p-8 relative z-20 flex flex-col h-[calc(100%-16rem)] min-h-[250px] justify-between">
                            <div>
                                <h3 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white mb-3 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-300">
                                    {project.title}
                                </h3>
                                <p className="text-slate-600 dark:text-slate-400 line-clamp-3 text-base leading-7 tracking-normal mb-6 transition-colors duration-300">
                                    {project.description}
                                </p>
                            </div>
                            ...
                            <div className="mt-auto">
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {project.tags.map((tag, tagIndex) => (
                                        <span
                                            key={tagIndex}
                                            className="px-3 py-1 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700/50 rounded-full transition-colors duration-300"
                                        >
                                            {tag}
                                        </span>
                                    ))}
